...
export default async (params: Params) => {
    ...
    if (args.has('bundle')) {
        ...
        currentPackage.scripts['clean-bundle'] = 'vamtiger-remove -d build -p \"(index|bin).(js(.map)?|d.ts)\"'.replace('.', '\\\\.');
        ...
        currentPackage.scripts['bundle-html'] = 'vamtiger-bundle-html --relativePath --entryFolder source/html --bundleFilePath --bundleFilePath source/html/index.ts --ignore source/test/.*.html$';
